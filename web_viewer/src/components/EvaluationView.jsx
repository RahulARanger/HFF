...
function JobCard({ job }) {
  ...
  const averages = summary.average_metrics || {};
  return (
    <Card className="evaluation-job-card">
      ...
      <CardContent>
        ...
        {job.status === "completed" && <div className="evaluation-average-grid">
          {Object.entries(averages).map(([branch, metrics]) => <div key={branch}><span>Average {branch.replace("_", " ")}</span><strong>{metrics.map((metric) => metric === null ? "—" : Number(metric).toFixed(4)).join(" · ")}</strong></div>)}
        </div>}
