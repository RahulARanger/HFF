...
export default function ValidationMonitorView() {
  ...
  const handleDelete = async (run) => {
    ...
    const isActive = ["queued", "running"].includes(run.status);
    const confirmed = window.confirm(isActive
      ? `Remove the ${run.status} validation monitor for "${run.label}"? This hides the monitor record and removes its telemetry files, but does not cancel the PBS/evaluation process or delete its outputs. The job may recreate its record while it continues running.`
      : `Delete validation telemetry for "${run.label}"? This removes the monitor record and resource logs, but keeps evaluation outputs and checkpoints.`);
    if (!confirmed) return;
