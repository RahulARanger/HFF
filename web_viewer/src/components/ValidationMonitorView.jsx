...
import { fetchValidationRun, fetchValidationRuns } from "../api.js";
...
function ValidationRunCard({ run, selected, onSelect }) {
  const progress = run.status === "completed" ? 100 : run.status === "running" ? 50 : 0;
  return <button type="button" className={`monitor-run-card ${selected ? "selected" : ""}`} onClick={() => onSelect(run.id)}><div className="monitor-run-card-topline"><span className="monitor-run-label">{run.label}</span><StatusBadge status={run.status} /></div><div className="monitor-run-card-meta"><span>{run.backend} · {run.sample_count || 0} samples</span><span className="run-arrow">›</span></div><div className="run-progress-line"><span style={{ width: `${progress}%` }} /></div><div className="monitor-run-card-epochs"><span>{run.request?.checkpoints?.length || 0} checkpoints</span><span>{run.timing?.elapsed_display || formatDuration(run.timing?.elapsed_seconds)}</span></div><div className="monitor-run-card-values"><span>RAM <strong>{formatBytes(run.latest?.ram_rss_bytes)}</strong></span><span>VRAM <strong>{formatBytes(run.latest?.gpu_memory_bytes)}</strong></span></div></button>;
}
